...
# ==============================
@app.get("/curso/alunos", summary="Consultar alunos por curso (Todos novos e Egressos)")
...
# ROTA PRINCIPAL
# ==============================
@app.get("/turma", summary="Dados de turmas com alunos matriculados por período letivo e curso")
def get_turma(
    codcurso: str = Query(..., description="Código do curso"),
    idperlet: str = Query(..., description="ID do período letivo"),
):
    try:
        with get_connection() as conn:
            ...
            if not rows:
                return {"erro": "Nenhuma turma encontrada"}

            # Converte cada linha em dicionário
            turmas = [dict(zip(columns, row)) for row in rows]
